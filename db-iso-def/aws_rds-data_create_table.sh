...
aws rds-data execute-statement --resource-arn ${clusterARN} --database ${RDS_DATABASE_NAME} --secret-arn ${dbSecretsARN} --sql "create table if not exists iso_array_int (vector  int[][])"
